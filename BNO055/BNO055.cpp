...
BNO055::BNO055 (I2C* _i2c_p, PinName p_interrupt, uint8_t addr, uint8_t mode) :
   i2c_p(_i2c_p), _interrupt(p_interrupt)
{
	chip_addr = addr;
	chip_mode = mode;
	initialize ();
...
void BNO055::get_quaternion(BNO055_QUATERNION_TypeDef &qua)
{
	...
	i2c_p->write(chip_addr, dt, 1, true);
	i2c_p->read(chip_addr, dt, 8, false);
	qua.w = dt[1] << 8 | dt[0];
	qua.x = dt[3] << 8 | dt[2];
	qua.y = dt[5] << 8 | dt[4];
	qua.z = dt[7] << 8 | dt[6];
...
void BNO055::get_chip_temperature(BNO055_TEMPERATURE_TypeDef *tmp)
{
	...
	dt[1] = 0;
	i2c_p->write(chip_addr, dt, 2, false);
...
void BNO055::initialize (void)
{
   ...
   i2c_p->frequency(100000);
#else
   i2c_p->frequency(400000);
#endif
	page_flag = 0xff;
	select_page(0);
...
void BNO055::check_id(void)
{
	...
	dt[0] = BNO055_CHIP_ID;
	i2c_p->write(chip_addr, dt, 1, true);
	i2c_p->read(chip_addr, dt, 7, false);
	chip_id = dt[0];
	if (chip_id == I_AM_BNO055_CHIP) {
		ready_flag = 1;
	} else {
...
void BNO055::set_mounting_position(uint8_t position)
{
	...
	dt[1] = remap_config;
	dt[2] = remap_sign;
   i2c_p->write(chip_addr, dt, 3, false);
